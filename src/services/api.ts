...
export const trainService = {
  searchTrains: async (params: { from_station_name: string; to_station_name: string; date: string }) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_TRAIN_API_URL}/api/trains/get`, params );
      ...
      console.log('pr', paramsWithoutDate);
      const r_price = await axios.post(`${import.meta.env.VITE_PRICE_API_URL}/api/price/stationids`,  paramsWithoutDate );
  ...
  getAvailableSeats: async (trainId: string, date: string) => {
    try {
      const response = await axios.get(`{import.meta.env.VITE_TRAIN_API_URL}/api/trains/${trainId}/seats`, {
        params: { date },
      });
  ...
  bookSeats: async (trainId: string, seats: string[], date: string) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_TRAIN_API_URL}/api/trains/${trainId}/book`, {
        seats,
        date,
      });
      return response.data;
